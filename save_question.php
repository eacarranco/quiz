...
if(empty($id)){
	...
	if($insert_question){
		...
		for($i = 0 ; $i < count($question_opt);$i++){
			// Convertir explícitamente a int para asegurar valores 1 o 0
			$is_correct = isset($is_right[$i]) && intval($is_right[$i]) === 1 ? 1 : 0;
			$insert[] = $conn->query("INSERT INTO question_opt set question_id = $question_id, option_txt = '".addslashes($question_opt[$i])."',`is_right` = $is_correct ");
		}
		if(count($insert) == 4){
			echo 1;
		}else{
	...
	if($update){
		...
		for($i = 0 ; $i < count($question_opt);$i++){
			// Convertir explícitamente a int para asegurar valores 1 o 0
			$is_correct = isset($is_right[$i]) && intval($is_right[$i]) === 1 ? 1 : 0;
			$insert[] = $conn->query("INSERT INTO question_opt set question_id = $id, option_txt = '".addslashes($question_opt[$i])."',`is_right` = $is_correct ");
		}
		if(count($insert) == 4){
			echo 1;
		}else{
